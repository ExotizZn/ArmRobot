...
#define NB_CURSEURS 6
#define NB_BOUTONS 6
#define NB_ETOILES 75
...
int demander_chemin_et_format(char *chemin, size_t max, int *binaire, int exporter) {
    FILE *fp = popen("zenity --list --radiolist --title='Format de fichier' "
                     "--column='' --column='Format' TRUE 'Texte' FALSE 'Binaire' --height=250 --width=300", "r");
    if(!fp) return 0;
    char buf[32];
...
int main(int argc, char *argv[]) {
    ...
    TTF_Init();
    srand(SDL_GetTicks());
    TTF_Font *police_titre = TTF_OpenFont("AVENGEANCE HEROIC AVENGER.ttf", 66);
    if(!police_titre) { printf("Police titre manquante\n"); return 1; }
    TTF_Font *police_etiquette = TTF_OpenFont("Orbitron-VariableFont_wght.ttf", 32);
    if(!police_etiquette) { printf("Police label slider manquante\n"); return 1; }
    TTF_Font *police_valeur = TTF_OpenFont("Orbitron-VariableFont_wght.ttf", 24);
    if(!police_valeur) { printf("Police valeur manquante\n"); return 1; }
    TTF_Font *police_bouton = TTF_OpenFont("space age.ttf", 24.5);
    if(!police_bouton) { printf("Police bouton manquante\n"); return 1; }
    char *etiquettes[NB_CURSEURS] = {"Base", "Épaule", "Coude", "Poignet", "Pinces", "Rotation"};
    SDL_Color paires_couleurs[NB_CURSEURS][2] = {
    ...
    const char *textes_boutons[NB_BOUTONS] = {
