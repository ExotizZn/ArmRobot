...
int main(int argc, char *argv[]) {
	...
	CachedText cached_title_chars[20]; // Enough for "Articulated RobotArm" (19 chars + null)
	CachedText cached_button_texts[NB_BOUTONS];
	CachedText cached_cursor_labels[NB_CURSEURS];
	...
	const Uint32 serial_update_interval = 100;
	...
	while (!quitter) {
		...
		if ((int)curseurs[0].valeur != temp1 || (int)curseurs[1].valeur != temp2 || 
			...
			if (current_ticks - last_serial_update >= serial_update_interval) {
				char cmd[100];
				snprintf(cmd, sizeof(cmd),
					"M1:%d,M2:%d,M3:%d,M4:%d,M5:%d,M6:%d\r\n",
					(int)curseurs[5].valeur, (int)curseurs[4].valeur, (int)curseurs[0].valeur,
					(int)curseurs[2].valeur, (int)curseurs[1].valeur, (int)curseurs[3].valeur);
	...
	for (int i = 0; i < NB_BOUTONS; i++) SDL_DestroyTexture(cached_button_texts[i].texture);
	for (int i = 0; i < NB_CURSEURS; i++) SDL_DestroyTexture(cached_cursor_labels[i].texture);
